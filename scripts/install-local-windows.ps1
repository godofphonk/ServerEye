# ServerEye Agent Local Installation Script for Windows
# This script installs and configures ServerEye agent using local binary file
...
Write-Host "🚀 Installing ServerEye Agent for Windows (Local)..." -ForegroundColor Green
...
try {
    New-NetFirewallRule -DisplayName "ServerEye Agent" -Direction Outbound -Program "$InstallDir\servereye-agent.exe" -Action Allow -Protocol TCP -LocalPort Any | Out-Null
